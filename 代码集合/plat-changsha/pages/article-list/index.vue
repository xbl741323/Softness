<template>
  <div class="article_list">
    ...
    <!-- 主体内容区域 -->
    <div :class="['article_content',bannerList.length>0?'':'no_banner_sty']">
      <!-- 导航标签区域 -->
      <div class="nav_contain">
        <nav-search ref="search" @search='refreshPage()'></nav-search>
      </div>
      <!-- 列表区域 -->
      <div class="article_list_contain" v-loading.lock="loading" v-if="dataList.length > 0">
        <article-item
      ...
      <div class="article_page">
        <div class="page">
          ...
          <a v-else :href="'/article-list?id='+$route.query.id+'&type='+$route.query.type+'&title='+keyWords+'&page='+pagination.previous+'&oneId='+$route.query.oneId" class="prev-btn light-page-block">上一页</a>
          <a v-for="(page, pageIndex) in pagination.pages" :key="pageIndex" :href="'/article-list?id='+$route.query.id+'&type='+$route.query.type+'&title='+keyWords+'&page='+page+'&oneId='+$route.query.oneId" :class="pagination.current == page ? 'current' : ''">{{ page }}</a>
          <span v-if="point" class="point-sty">...</span>
          <a v-if="pagination.last !==pagination.pages[pagination.pages.length-1]&&pagination.last !== (pagination.next + 1) && pagination.last !== pagination.next && pagination.current+2 < pagination.last" :href="'/article-list?id='+$route.query.id+'&type='+$route.query.type+'&title='+keyWords+'&page='+pagination.last+'&oneId='+$route.query.oneId" class="light-page-block">{{ pagination.last }}
          </a>
          <a v-if="pagination.current === pagination.last" class="prev-btn dis light-page-block">下一页</a>
          <a v-else class="prev-btn light-page-block"  :href="'/article-list?id='+$route.query.id+'&type='+$route.query.type+'&title='+keyWords+'&page='+pagination.next+'&oneId='+$route.query.oneId">下一页</a>         
        </div>
      </div>
    </div>
  </div>
