    ...
    <div :class="[showStatus == true ? 'nutice_filter' : 'filter_more']">
      <popup v-model="currentValue">
        <filter-filter 
          ...
          :type="type"
          :rankData="rankData" 
          :active="showIndex==1?industryType:comprehensiveSort"
...
<script>
import {esPageIndustry } from "@/api/policySquare";
...
export default {
  ...
  data() {
    return {
      ...
      total: 0,
      pageSize: 50,
      title: this.$t("message.industryAnswer"),
      nuticeArr: [],
      keyWords: "",
      start: 1,
      isNewest: true,
  ...
  methods: {
    ...
    changeStatus(val, index) {
      ...
      if(this.showIndex==1){
        this.rankData = policyData.classList
      }else if(this.showIndex==2){
        this.rankData = policyData.newSortData
      }
    },
    getSearch(val) {
      this.keyWords = val;
      if(val!=''){
        ...
        this.selectNameList[1].categoryName = '综合排序'
        this.selectNameList[1].status = true
      }
      this.start = 1;
      this.getListData();
    ...
    getListData() {
        ...
        this.params = {
          ...
          categoryId: this.industryType == 0 ? null : this.industryType,
        };
        sessionStorage.setItem("projectParams",JSON.stringify(this.params))
        esPageIndustry(this.params).then((res) => {
          this.loading = false
          if(res.code == 0){
            this.total = res.data.total;
            this.nuticeArr = res.data.industryInfoList;
            if (res.data.industryInfoList.length == 0) {
              this.showMsg = 1;
            } else {
              this.showMsg = 0;
            }
