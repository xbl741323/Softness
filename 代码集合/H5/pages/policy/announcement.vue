    ...
    <div :class="[showStatus == true ? 'nutice_filter' : 'filter_more']">
      <popup v-model="currentValue" >
        <filter-filter 
          ...
          :type="type"
          :rankData="rankData" 
          :active="showIndex==1?localAreaNo:showIndex==2?level:showIndex==3?putunder:showIndex==4?applyTime:comprehensiveSort"
...
export default {
  ...
  data() {
    return {
      ...
      total: 0,
      pageSize: 50,
      title: this.$t("message.announcements"),
      nuticeArr: [],
      keyWords: "",
      start: 1,
      isNewest: true,
      ...
      rankData: [],
      selectNameList: policyData.newSelectNameList,
      selectList: [],
      showIndex: "",
      currentValue: false,
      region: [],
  ...
  methods: {
    ...
    searchFilter(val) {
      ...
      switch(Number(this.showIndex)){
        ...
        case 3:
          ...
          break;
         case 4:
          this.applyTime = val.no
          if(val.no == 0){
              this.selectNameList[3].categoryName = this.selectNameList[3].otherName
              this.selectNameList[3].status = false
            }else{
              this.selectNameList[3].categoryName = val.name
              this.selectNameList[3].status = true
          }
          break;
        case 5:
          this.comprehensiveSort = val.no
          if(val.no == 0){
              this.selectNameList[4].categoryName = val.name
              this.selectNameList[4].status = false
            }else{
              this.selectNameList[4].categoryName = val.name
              this.selectNameList[4].status = true
          }
          break;
      }
      this.getListData();
    ...
    changeStatus(val, index) {
      ...
      if(this.showIndex==1){
        ...
        this.rankData = policyData.levelData
      }else if(this.showIndex==3){
        this.rankData = this.classList
      }else if(this.showIndex==4){
        this.rankData = policyData.monthData
      }else{
        this.rankData = this.$route.path=='/policy/announcement'?policyData.newSortData : policyData.sortData
      }
    },
    getNameList() {
    ...
    getSearch(val) {
      ...
      if(val!=''){
        ...
        this.selectNameList[4].categoryName = '综合排序'
        this.selectNameList[4].status = true
      }
      this.getListData();
    },
    ...
    getListData() {
        ...
        this.params = {
          ...
          sort: this.comprehensiveSort == 0 ? 'latest' : 'hottest',
          optionId: this.putunder == 0 ? null : this.putunder,
          level: this.level == 0 ? '全部' : this.level,
