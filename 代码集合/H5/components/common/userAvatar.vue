<template>
  <div class="user_avatar">
    <!-- <img v-if="url.length<100" :src="url ? url + '?t=' + (new Date().getTim``-e()) : ''" title="点击上传头像" class="avatar_img" /> -->
    <img  :src="url" class="avatar_img" @click="open=true" />
    <el-dialog  :visible.sync="open" width="100%"  :close-on-click-modal="true" @close="open = false">
      <el-row>
...
export default {
  ...
  data() {
    return {
      avatar: "https://static.yagu.cn/assets/icons/default.png",
      // 是否显示弹出层
      open: false,
      // 弹出层标题
      options: {
