<template>
  <div class="project_collect">
    ...
    <div @click="changeTitle" style="height: 40px;line-height: 40px;padding-left: 15px;background:#ffffff">
      ...
      <span v-if="showStatus == 1" class="bbbbbb">正常</span>
      <span v-if="showStatus == 2" class="bbbbbb">已失效</span>
      <i class="el-icon-arrow-down bbbbbb" v-show="!showState"></i>
      <i class="el-icon-arrow-up bbbbbb" v-show="showState"></i>
    </div>
    <div style="border:1px solid #cccccc0a"></div>
    <div style="display:flex;padding-left: 16px;padding-top: 10px;background:#ffffff" v-show="showState">
        ...
        <div :class="['stateotherbtn',showStatus == 1?'statebtn':'']" @click="changeStatus(1)" style="margin-left:15px">正常</div>
        <div :class="['stateotherbtn',showStatus == 2?'statebtn':'']" @click="changeStatus(2)" style="margin-left:15px">已失效</div>
    </div>
...
<script>
import resultItem from "@/components/collect/propertyItem";
import { getProCollectList } from "@/api/property";
export default {
  components: {
    resultItem,
  },
  ...
  methods: {
    ...
    getList() {
      ...
      getProCollectList(params).then((res) => {
