    ...
    methods: {
      ...
      menuTag(value, action) {
        if (action === 'remove') {
            ...
            // 关键字搜索
            if(sessionStorage.getItem(`${value}special`)){
                sessionStorage.removeItem(`${value}special`)
            }
      ...
      getList() {
        fetchMenuTree(false).then(response => {
          ...
          this.searchPath(menuList);
        });
      },
      searchPath(val){
        if (val.length > 0) {
          val.forEach(item => {   
              if(item.path=="/personal/myHome"){
                localStorage.setItem('globalLabel', item.label);
              } else {
