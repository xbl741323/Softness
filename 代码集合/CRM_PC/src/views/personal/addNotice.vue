  ...
  <div class="notice_add" v-loading.fullscreen.lock="fullscreenLoading">
    <div>
      ...
      <div style="text-align: center; margin: 30px 0">
        <el-button type="success" v-if="type != 4 && permissions.bt_add_information" @click="publishNotice('form', 0)" v-preventReClick>提交审核</el-button>
        <el-button @click="publishNotice('form', 1)" v-if="type == 4 && permissions.bt_add_information" type="primary" v-preventReClick>{{ $t("button.submit") }}</el-button>
        <el-button type="warning" @click="previewNotice">{{$t("button.preview")}}</el-button>
        <el-button @click="draft('form')" v-if="type != 4 && permissions.bt_save_draft_information" type="primary" v-preventReClick>{{$t("button.saveDraft")}}</el-button>
      </div>
      <!-- 添加关联产品 -->
...
import { saveDraft, addNotice, rechecking } from "@/api/personal/notice";
