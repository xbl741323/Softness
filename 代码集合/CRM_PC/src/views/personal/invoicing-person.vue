      ...
      <div class="details">
        ...
        <div class="top-detail">
          <div class="processnumber">
              ...
              <el-button type="primary" size="small" class="ceshi"  v-if="snapshotInformation.taskCurrentEmpId && currentUserId == snapshotInformation.taskCurrentEmpId && snapshotInformation.taskStatus == 1" @click="openDialog">修改发票信息</el-button>
              <span class="zuofei"  v-if="snapshotInformation.taskStatus == 6">已作废</span>
          </div>
          <table class="table" style="margin-bottom: 30px">
            <tr><th>流程状态</th><th>合计开票金额</th><th>费用类型</th><th>退款状态</th><th>发起人</th><th>发起时间</th>
            </tr>
            <tr>
              <td>
        ...
        <template v-if="snapshotInformation.headerType ==0">
            ...
            <div style="display:flex">
                ...
                <div class="two" :class="{'dbottom':!snapshotInformation.invoiceUrl}">{{ snapshotInformation.headerType? snapshotInformation.corporateName: snapshotInformation.personalName}}</div>
        ...
        <template v-else>
            ...
            <div style="display:flex">
                ...
                <el-tooltip effect="dark" :content="snapshotInformation.invoiceAddress" placement="right">
                <span>{{ snapshotInformation.invoiceAddress.substring(0,26) }}...</span>
                </el-tooltip>
                </div>
        ...
        <!-- 开票审批 -->
        <div class="examine"  v-if="snapshotInformation.taskCurrentEmpId && currentUserId == snapshotInformation.taskCurrentEmpId && snapshotInformation.taskStatus == 0">
          ...
          <template v-if="snapshotInformation.isRefund !=0">
            ...
            <div class="examine-content">
                ...
                <div class="button">
                <el-button type="success" size="medium" v-preventReClick  @click="handlePass">{{ $t("button.auditingPassed") }}</el-button>
                <el-button type="danger" size="medium" class="unpassed" v-preventReClick  @click="handleUnpass" >{{ $t("button.auditingUnPassed") }}</el-button>
                </div>
            </div>
          </template>
        </div>
...
<script>
import { getpersoninvoicingdDetail,getCompany,audit,auditFail,editPersonInvoicing } from "@/api/processmanage/invoicing";
...
export default {
  ...
  methods: {
    ...
    handleEdit(formName){
       if(!this.invoiceUrl){
         ...
         this.$refs[formName].validate((valid) => {
          if (valid) {
             ...
             editPersonInvoicing(param).then(res=>{
                if (res.data.code == CodeMsg.CODE_0) {
                    this.$message({
                        type: "success",
                        message: "修改成功!"
     ...
     handleClose(){
        this.dialogVisible = false
        this.invoicingfile = null
        this.invoicingfileName = null
        this.invoicingForm = {}
    }
  },
};
