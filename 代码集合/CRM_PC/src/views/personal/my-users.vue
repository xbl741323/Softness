  ...
  <basic-container>
    <div class="user_list">
      ...
      <div class="batch-handle">
          <el-button size="small" @click="handledistribution(0,{},1)" type="primary" :disabled="batchList.length==0">分配客服</el-button>
      </div>
      </div>
      <!-- 单位 -->
      <div v-show="filters.account == 0" class="unit">          
       <el-table ref="crud" :data="userList"   border row-key="accountNo"  :tree-props="{children: 'accounts'}"
         v-loading="tableLoading" class="batch-choose unit-user" @selection-change="selectionChange">
        <el-table-column type="selection"  :selectable="selectAble" align="center"></el-table-column>
        <!-- 主体ID -->
        <el-table-column prop="userNo" label="主体ID" width="120" align="center" show-overflow-tooltip > 
        ...
        <!-- 线索编号 -->
        <el-table-column prop="clueNo" label="线索编号" width="160" align="center" show-overflow-tooltip></el-table-column>
        <!-- 单位名称 -->
        <el-table-column prop="companyName" label="单位名称" width="200" align="center" show-overflow-tooltip> 
          <template slot-scope="scope">
            ...
            <img :src="require('@/assets/images/new-client/certified_unit_low.png')" alt="" style="width: 20px;height: 20px; margin-left: 3px" v-if="scope.row.authorizedType==1">
            <img :src="require('@/assets/images/new-client/certified_unit.png')" alt="" style="width: 20px;height: 20px; margin-left: 3px" v-if="scope.row.authorizedType==2">
          </template> 
        </el-table-column>
        <!-- 登录ID -->
        ...
        <!-- 注册时间 -->
        <el-table-column prop="regCreateTime" label="注册时间" align="center" width="140"></el-table-column>
        <!-- 操作 -->
        <el-table-column label="操作" align="center" width="100"> 
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="handleView(scope.row)">查看</el-button>
            <el-button type="text" size="small" @click="follow(scope.row)" v-preventReClick>跟进</el-button>
            <!-- <el-button type="text" size="small" @click="handledistribution(1,scope.row)">分配</el-button> -->
            <!-- <el-button type="text" size="small" v-if="scope.row.isDisabled==0" @click="handleEnabledisable(scope.row)">禁用</el-button>
            <el-button type="text" size="small" v-if="scope.row.isDisabled==1" @click="handleEnabledisable(scope.row)">启用</el-button> -->
          </template>
        </el-table-column>
      </el-table>
      <div class="textAli">
      ...
      <el-dialog  :title="dialogTitle"  :visible.sync="dialogVisible" :close-on-click-modal="false" width="23%" :before-close="handlebeforeDialogClose">
        <el-form :model="userForm" ref="userForm" :rules="rules" label-width="140px">
            ...
            <el-form-item label="请选择推广归属：" prop="promoteAttributionName" v-if="(which == 0&&allotType == 2)||(which == 1)">
            <el-cascader filterable :show-all-levels='false' v-model="userForm.promoteAttributionName" :options="empList" :props="{value:'customerId'}"></el-cascader>
            </el-form-item>    
        </el-form>
...
import { mygetUserList,Enabledisable,distribution} from "@/api/restructureuser/index"
...
export default {
  ...
  methods: {
    ...
    getList() {
        ...
        mygetUserList(this.filters.account,param).then(res=> {
            if(res.data.code==0){
