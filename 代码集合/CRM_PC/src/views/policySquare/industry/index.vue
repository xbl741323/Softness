...
export default {
  ...
  created(){
    ...
    if(sessionStorage.getItem(`${this.$route.path}`)){
      ...
      this.filters.showStatus = keepParams.params.state
      this.filters.cate = keepParams.params.categoryId
  ...
  methods: {
    ...
    changeStatus(value) {
      ...
      this.$confirm(this.$t("alert.editStatus"), this.$t("title.notice"), {
        ...
        .then(() => {
         listChangeStatus(para,'list').then((res) => {
           if(res.data.code == CodeMsg.CODE_1000){
             this.$message({
              type: "success",
    ...
    getList() {
      ...
      industryList(params, 'list').then((res) => {
        if (res.data.code == CodeMsg.CODE_1000) {
    ...
    handleDelete(row, index) {
      this.$confirm('是否确认删除"' + row.title + '"的行业资讯?', "警告", {
        ...
        deleteIndustry(row.industryNo, 'list').then(res=>{
          if(res.data.code == CodeMsg.CODE_1000){
            this.$message({
            type: "success",
            message: "删除成功！",
