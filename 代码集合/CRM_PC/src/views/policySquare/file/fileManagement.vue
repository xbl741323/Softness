<template>
  <div
    ...
    v-if="permissions.bt_page_doc || permissions.bt_edit_doc || permissions.bt_del_doc || permissions.bt_download_doc"
  >
    <el-form :inline="true" :model="fileForm" @submit.native.prevent>
      ...
      <el-button size="small" type @click="permissions.bt_add_doc ? showDialog() : $notAuthorized()">{{$t('button.uploadFile')}}</el-button>
    </el-form>
    <div class="batch-handle">
      ...
      <!-- <el-button size="mini" type="success" :disabled="this.selectedNum==0" @click="permissions.bt_edit_doc ? BatchcellReplaceFile(selectList) : $notAuthorized()">替换</el-button> -->
      <el-button
        size="mini"
        type="primary"
        @click="permissions.bt_edit_doc ? statusDialog = true : $notAuthorized()"
        :disabled="this.selectedNum==0"
      >{{$t('button.showStatus')}}</el-button>
      <el-button
        size="mini"
        type="warning"
        @click="permissions.bt_del_doc ? deleteDialog = true : $notAuthorized()"
        :disabled="this.selectedNum==0"
    ...
    >
      ...
      <template slot="state" slot-scope="scope">
        ...
        <el-select
          ...
          v-else
          :disabled="scope.row.state == -1 || !permissions.bt_edit_doc"
        >
      ...
      <template slot="menu" slot-scope="scope">
        ...
        <el-button type="text" size="small" @click="permissions.bt_download_doc ? handleEdit(scope.row) : $notAuthorized()">编辑</el-button>
        <!-- <el-button type="text" size="small" icon="el-icon-change" v-if="scope.row.deleted !==1" @click="permissions.bt_edit_doc ? cellReplaceFile([scope.row]) : $notAuthorized()">替换</el-button> -->
        ...
        <el-button type="text" size="small" @click="permissions.bt_download_doc ? downLoad(scope.row) : $notAuthorized()">{{$t('button.download')}}</el-button>
        <el-button  type="text" size="small" v-if="scope.row.deleted != 1" @click="permissions.bt_del_doc ? deleteFile(scope.row) : $notAuthorized()">{{$t('button.delete')}}</el-button>
      </template>
    </avue-crud>
   <!-- 文件替换 -->
    ...
    <el-dialog :visible.sync="uploadDialog" width="45%" :close-on-click-modal="false" center @close="cancleUpload">
      ...
      <div class="upload">
        ...
        <!-- <el-button size="small" type="primary" @click="deleteRow(fileObj)" class="button-class" v-if="fileObj.name"> </el-button> -->
        <el-form ref="fileForm" label-width="100px" :rules="inputRules" v-if="fileObj.name" style="margin-top: 20px;">
          <el-form-item label="文件名称：" prop="name">
            <edit-box :row="fileObj" :permissions="permissions.bt_edit_doc" @saveKeywords="childSave" @deleteRow=deleteRow class="editBox"></edit-box>
          </el-form-item>
...
import { getDocList, removeFile, batchShow, getNewArea, addDoc, downloadDoc} from "@/api/policy/list";
...
export default {
  ...
  created(){
    if(!this.permissions.bt_page_doc && !this.permissions.bt_edit_doc && !this.permissions.bt_del_doc && !this.permissions.bt_download_doc) this.$notAuthorized()
    if(sessionStorage.getItem(`${this.$route.path}`)){
      ...
      this.fileForm.uploader = keepParams.params.operatorNoOrName
      this.fileForm.showStatus = keepParams.params.state
      this.fileForm.level = keepParams.params.level
      this.fileForm.area = keepParams.params.areaNo ? keepParams.params.areaNo.split(',') : ""
  ...
  methods: {
    ...
    async uploadFile(file,fileName) {
      ...
      const uploader = await new OSSUploader()
    ...
    async downLoad(row) {
      ...
      if (row.pages == 0) {
        ...
        await downloadDoc(row.id).then(res => {
          if (res.data.code == 0) {
            url = res.data.data
          }
        }).catch(err => {
