<template>
  <div class="file_contain">
        ...
        <td class="td-file" >
          <div class="edit-box">
            ...
            <!-- <el-tooltip effect="light" :content="fileDetail.fileName" placement="left-bottom" :open-delay="500" v-else>
              <edit-box :row="fileDetail" :permissions="permissions.bt_page_doc ? true : false" @saveKeywords='childSave' style="margin:0" :width="700"></edit-box>
            </el-tooltip> -->
          </div>
    ...
    <div class="related-articles">
      ...
      <!-- <el-button :disabled="this.selectedNum==0" @click="permissions.bt_page_doc ? cellReplaceFile(selectList) : $notAuthorized()">替换</el-button> -->
      ...
      <el-table :data="articleList" border @selection-change="handleSelectionChange" :header-cell-style="{'text-align':'center'}" class="table">
        ...
        <el-table-column label="操作" width="150" align="center">
          <template slot-scope="scope">
            <el-button type="text" @click="view(scope.row)" :disabled="type != 1">浏览</el-button>
            <!-- <el-button type="text" @click="permissions.bt_page_doc ? cellReplaceFile([scope.row]) : $notAuthorized()">替换</el-button> -->
      ...
      <!-- 文件替换 -->
      <file-replace-dialog :uploadDialog="replaceDialog" @closeDialog='closeReplace()' :cellData='cellReplaceData' :source="source" @handleReplace="handleReplace" v-if="replaceDialog"></file-replace-dialog>
    </div>
  </div>
   </div>
</template>
...
import { releateArticle,deleteRelate,docDetail, editDoc } from '@/api/policy/detail'
...
export default {
  ...
  methods:{
    ...
    handleBack() {
      this.$router.push('/policySquare/file/fileManagement')
    },
    closeReplace(val){
