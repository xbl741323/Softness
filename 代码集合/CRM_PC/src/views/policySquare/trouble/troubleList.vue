    ...
    <div class="avue-crud">
        ...
        <div style="display: flex">
          ...
          <div>
            ...
            <el-button
              icon="el-icon-plus"
              type="success"
              @click="permissions.sys_problem_edit ? addArticle() : $notAuthorized()"
      ...
      <div>
        <div class="batch-handle">
          ...
          <el-button icon="el-icon-view" type="primary" :disabled="this.batchList.length==0" @click="permissions.sys_problem_edit ? statusDialog = true : $notAuthorized()">显示状态</el-button>
          <el-button icon="el-icon-refresh-right" type="success" @click="permissions.sys_problem_edit ? transferDialog = true : $notAuthorized()" :disabled="this.batchList.length==0">转移</el-button>
          <el-button icon="el-icon-delete" type="danger" @click="permissions.sys_problem_del ? deleteBatch() : $notAuthorized()" :disabled="this.batchList.length==0">删除</el-button>
        </div>
        <avue-crud
          class="batch-choose"
          ref="crud"
          ...
          <template slot="number" slot-scope="scope">
            <span
              @click="permissions.sys_problem_edit ? handleEdit(scope.row,scope.index) : $notAuthorized()"
          ...
          <template slot="showStatus" slot-scope="scope">
            ...
            <div v-else>
              <el-select
                ...
                :disabled="scope.row.deleted==1 || !permissions.sys_problem_edit"
          ...
          <template slot="menu" slot-scope="scope">
            ...
            <div v-else>
              ...
              <el-button type="text" size="small" icon="el-icon-link" @click="permissions.sys_problem_edit ? getCodeLink(scope.row) : $notAuthorized()" v-if="scope.row.showStatus == 0 || scope.row.showStatus == 1" :data-clipboard-text="scope.row.title+'：'+link" class="link">推广</el-button>
              <el-button type="text" size="small" icon="el-icon-edit" @click="permissions.sys_problem_edit ? fastEdit(scope.row, scope.index) : $notAuthorized()">{{ $t("button.amend") }}</el-button>
              <el-button type="text" size="small" icon="el-icon-delete" @click="permissions.sys_problem_del ? handleDelete(scope.row, scope.index) : $notAuthorized()">{{ $t("button.delete") }}</el-button>
            </div>
          </template>
        </avue-crud>
        <!-- 快速修改弹框 -->
...
<script>
import { tableOption } from "@/const/crud/policy/trouble";
import { editNotice, delNotice, policyList, batchShowStatus, batchDelete, batchTransfer, listChangeStatus, getrapidChange} from "@/api/policy/problem";
...
export default {
  ...
  created(){
    if(!this.permissions.sys_problem_detail && !this.permissions.sys_problem_edit && !this.permissions.sys_problem_del) this.$notAuthorized()
  ...
  methods: {
    ...
    getCodeLink(row){
      this.link = process.env.VUE_APP_WEB_URL+'policy/'+ row.number+'21' + '?' + 'link=' +this.code; 
    ...
    showPrinceChild() {
      console.log(this.princeChild, "输出筛选的地区");
      this.getAllList()
    },
    showProvince(val) {
      console.log(val, "输出选择的地区");
      this.princeChild = [];
       this.princeCopyChild = []
     if (val.label == "全国") {
         this.princeInfo = {}
        ...
        this.princeInfo = val;
        console.log(this.princeInfo, "输出所有的地区");
        this.getAllList();
      }
    },
      showTime(val) {
    ...
    getList() {
      ...
      policyList(params).then(res => {
        ...
        let otherOptions = [
          ...
          {label: '标题',prop: 'title',width:350,overHidden: true,slot:true,}, 
          {label: '问题类型',prop: 'questionType',overHidden: true,slot:true,}, 
          {label: '级别',prop: 'level',overHidden: true,slot:true,}, 
          {label: '地区',prop: 'area',overHidden: true,slot:true,width: 150}, 
          {label: '发布人', prop: 'createUserName',overHidden:true,slot:true},
          {label: '发布时间', prop: 'createTime',type: 'datetime',overHidden:true,format: 'yyyy-MM-dd HH:mm',valueFormat: 'yyyy-MM-dd HH:mm:ss'},
