...
<script>
import { saveProblemDraft, addProblem, rechecking } from "@/api/policy/notice";
...
import { dropEditData, getDropPolicy ,getNewArea} from "@/api/policy/list";
...
export default {
  ...
  methods: {
    ...
    draft(formName) {
      if (this.checkFlieRepet) {
        ...
        if (this.edited) {
          ...
          saveProblemDraft(this.param, config)
            .then((res) => {
    ...
    selectData(val) {
      this.optionList = [];
      console.log(this.form.belongData,"疏忽")
      for (let i = 0; i < this.form.belongData.length; i++) {
        console.log(
          "selectData",
          JSON.stringify(this.form.belongData[i].itemData)
        );
        ...
        this.$refs[formName].validate((valid) => {
          if ( valid && this.editorContent.trim() != "") {
            if (this.productImg != null || this.type != 4) {
              ...
              addProblem(this.param, config).then((res) => {
                  if (res.data.code == CodeMsg.CODE_1000) {
    ...
    getDropdown() {
      getDropPolicy(this.type).then((res) => {
          ...
          console.log("输出获取的数据", this.form.belongData);
          // this.$nextTick(() => {
