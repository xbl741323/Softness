...
<template>
    <div class="js-column">
      ...
      <span class="message">您所添加的导航会在软件开发站前台导航进行展示</span>
      <avue-crud 
      ...
      :span-method="spanMethod2">
        ...
        <template slot="powerMan" slot-scope="scope" v-if="scope.row.permissionName!==null&&scope.row.permissionName!==''">
        ...
        <template slot="menu" slot-scope="scope">
          ...
          <el-button size="mini" type="text" v-if="scope.row.type!=4 &&scope.row.type!=5&&scope.row.type!=6"  @click="editColumns(1,scope.row.id)" icon="el-icon-edit" >编辑</el-button>
      ...
      <el-dialog :title="title" :visible.sync="dialogVisible" @close="closeForm" :close-on-click-modal='false' width="1120px" center >  
        <columns-form v-if="dialogVisible" @closeForm="closeForm" :columnId="columnId"></columns-form>
      </el-dialog>
      <!-- 权限设置 -->
...
<script>
import { tableOption } from "@/const/crud/js-wotao-b/columns-manage";
import {
  getColumnsList,
  deleteColumn,
  moveColumn,
...
import columnsForm from "@/components/nationalSite/columns.vue";
