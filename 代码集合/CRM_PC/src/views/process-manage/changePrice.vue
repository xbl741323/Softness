<template>
  <basic-container>
    <div
      v-if="
        permissions.sys_change_price_detail ||
          permissions.sys_change_price_audit ||
    ...
    >
        ...
        <div>
          <el-form-item :label="$t('label.processState')" prop="state">
            <el-radio-group
              ...
              @change="stateChange"
            >
              <!-- this.processlistForm.flowStatus -->
      ...
      <avue-crud
        v-loading="loading"
        ref="crud"
        class="batch-choose min-h-table"
        :data="tableData"
        :option="listOption"
        :page.sync="page"
...
<script>
import { getChangeList, getByStatus } from "@/api/personal/myprocess";
...
export default {
  ...
  created() {
    if (
      !this.permissions.sys_change_price_detail &&
      !this.permissions.sys_change_price_audit &&
      !this.$route.path.includes("personal")
    )
      this.$notAuthorized();
  ...
  methods: {
    ...
    getData() {
      if (this.refundFrom || this.$route.path == "/personal/my-process") {
        this.processlistForm.flowStatus = null;
      }
    ...
    getprocessList() {
      ...
      getChangeList(params, this.refundFrom).then(res => {
        if (res.data.code == 0) {
          this.loading = false;
