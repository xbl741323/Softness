  ...
  <div :class="['frame', changewidth ? 'gutter-special' : 'gutter-normal']" v-loading="loading">
    <div class="contract-frame">
      ...
      <div class="add-product">
        <el-button type="primary" icon="el-icon-position" @click="chooseClue()" v-if="permissions.sys_contract_paper_detail || isPersonal == 'personal'">{{buttonName}}</el-button >
      </div>
      ...
      <div class="clue-infos">
        <p>{{ clueInfo.clueNo }}</p>
        <!-- 线索名称 -->
        <p v-if="clueType == 1">{{ clueInfo.companyName }}</p>
        <p v-else>{{ clueInfo.contactName }}</p>
        <!-- 联系人 -->
        <p v-if="clueInfo.clueCompanyContactList">{{ clueInfo.clueCompanyContactList[0].contactName }}</p>
        <p v-else>{{ clueInfo.contactName }}</p>
        <!-- 手机号 -->
        <p v-if="clueInfo.clueCompanyContactList">{{ clueInfo.clueCompanyContactList[0].mobile }}</p>
        <p v-else>{{ clueInfo.mobile }}</p>
        <!-- 业务员 -->
        <el-tooltip effect="dark" :content="'工号：' + cEmpNo + ' 部门：' + cDeptName" placement="top-start" v-if="clueInfo.clueSalesmanEmpId!=0">
          <p @mouseover="showUserInfo(clueInfo.clueSalesmanEmpId)">
            {{ clueInfo.clueSalesmanEmpName }}
          </p>
        </el-tooltip>
        <p v-else>{{ clueInfo.clueSalesmanEmpName }}</p>
      </div>
      <!-- 订单信息 -->
      <span class="subtitle">订单信息</span>
      <p class="order_txt">最多可添加20条订单信息</p>
      <div class="add-product">
        <el-button type="primary" icon="el-icon-plus" @click="addProduct()" v-if="permissions.sys_contract_paper_detail || isPersonal == 'personal'">添加订单</el-button>
      </div>
      ...
      <el-form :model="constractForm" ref="constractForm" :rules="rules">
        ...
        <el-form-item label="合同附件" prop="file" class="form-item">
          <div :class="constractForm.file ? 'newsty' : ''" v-if="permissions.sys_contract_paper_detail || isPersonal == 'personal'">
            <file-upload
              :limit="1"
              :acceptType="'.pdf'"
              :multipleFile="false"
        ...
        <el-form-item label="审批人：" prop="auditor" label-width="80px">
          <el-cascader v-if="whichselet==1" :show-all-levels='false'  :props="{ value: 'customerId', label: 'label'}" options="empList" filterable v-model="constractForm.auditor" placeholder="请选择审核人" lang="审核人" @change="auditorChange">
                <template slot-scope="{ node,data }">
                  <span>{{ data.label }}</span>
                  <span v-if="node.isLeaf && data.lockFlag && data.lockFlag ==9"> <img src="/img/disable.png" class="disablemancal" alt="" /></span>
                </template>
          </el-cascader>
          <el-select v-else-if="whichselet ==0" v-model="constractForm.auditor" ref="myselect">
            <el-option 
          ...
          <span v-else>{{constractForm.name}}</span>
        </el-form-item>
        <div class="btn">
...
import { getEmpList, getOneEmp } from "@/api/clue/clue";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      contactNoList: [],
      isPersonal: this.$route.query.source,
  ...
  created() {
    this.updateContract = this.$route.query.updateContract;
    if(this.$route.query.clueId){
      this.clueId = this.$route.query.clueId
      this.orderParam.clueId = this.$route.query.clueId
      this.clueType = this.$route.query.clueType
      this.orderParam.clueType = Number(this.$route.query.clueType)+1
    }
  ...
  methods: {
    auditorChange(){
      this.$nextTick(() => { 
        this.constractForm.auName = this.$refs["cas"].getCheckedNodes()[0].data.customerId;
      });
    },
    // 获取员工信息
    showUserInfo(id){
    ...
    getEmp() {
      ...
      getOneEmp().then((res) => {
        if (res.data.data.length == 0) {
          ...
          this.constractForm.auName= this.constractForm.name;
        } else {
          this.whichselet = 0;
    ...
    getData(){
        getPaperContractsDetail(Number(this.$route.query.id)).then((res) => {
          if (res.data.code == CodeMsg.CODE_0) {
            ...
            if(this.draftContractNo!=''){
              this.constractForm.contactNoTitle = 0
            }
            // this.constractForm.contractNote = res.data.data.remark
            this.$set(this.constractForm, "signName", res.data.data.signName);
            this.$set(this.constractForm, "signMobilephone", res.data.data.signMobile);
            this.$set(this.constractForm, "signTime", res.data.data.signTime);
            this.$set(this.constractForm, "contractNote", res.data.data.remark);
            if(res.data.data.handlerId){
              if(this.whichselet==0 || this.whichselet==1){
                this.$set(this.constractForm, "auditor", res.data.data.handlerId);
              }else{
                this.$set(this.constractForm, "auditor", 1);
              }
            }
            this.$set(this.clueInfo, "clueNo", res.data.data.clueNo);
            this.$set(this.clueInfo, "contactName", res.data.data.contactName);
            this.$set(this.clueInfo, "mobile", res.data.data.mobile);
            this.$set(this.clueInfo, "clueId", res.data.data.clueId);
            this.$set(this.clueInfo, "clueSalesmanEmpId", res.data.data.clueSalesmanEmpId);
            this.$set(this.clueInfo, "clueSalesmanEmpName", res.data.data.clueSalesmanEmpName);
            this.$set(this.orderParam, "clueType", res.data.data.clueType);
            this.$set(this.orderParam, "clueSalesmanEmpId", res.data.data.clueSalesmanEmpId);
            this.$set(this.orderParam, "clueId", res.data.data.clueId);
            if(res.data.data.orderIdList){
              res.data.data.orderIdList.length>0&&res.data.data.orderIdList.forEach((item, index)=>{
                this.$set(this.orderIdList, index, item)
              })
              this.getOrder(JSON.parse(JSON.stringify(res.data.data.orderIdList)))
            }
            this.contractStatus = res.data.data.taskStatus;
            this.$set(this.contractFile,0,{
              url: res.data.data.signUrl,
              name: res.data.data.contractName,
              file:new File([],res.data.data.contractName,{})
            });
            ...
            this.$set(this.constractForm, "file", {
              url: res.data.data.signUrl,
              name: res.data.data.contractName,
              file:new File([],res.data.data.contractName,{})
            });
            this.contractUrl = res.data.data.signUrl
    ...
    getClue(val) {
      ...
      this.orderParam.clueType = Number(val.clueType) + 1;
      this.clueType = val.clueType
    ...
    deleteProduct(row) {
      this.$confirm("确认删除该订单吗？", "提示", {
        ...
        for (let i = 0; i < this.form.orderList.length; i++) {
          ...
          if(this.orderIdList==undefined){
            this.orderIdList = []
            this.form.orderList.length>0 && this.orderIdList.push(this.form.orderList[i].id)
          }
        }
        this.orderIdList.forEach((item)=>{
          if(item == row.id){
            this.orderIdList.splice(this.orderIdList.indexOf(item),1);
          }
        })
        this.orderIdList = [...new Set(this.orderIdList)]
      });
    },
    submit() {
      //确认审核人
    ...
    submitAudit(isDraft) {
      ...
      var param = {
        isDraft: isDraft,
        id:null,
        clueId:  this.orderParam.clueId,//线索ID
        clueType:  this.orderParam.clueType,//认证类型 1个人 2企业
        salesmanEmpId: this.orderParam.clueSalesmanEmpId,//线索业务员ID
        orderIdList: JSON.parse(JSON.stringify(this.orderIdList)),//订单列表
        signUrl: this.contractUrl,//签署合同附件url
        signName:  this.judge(this.constractForm.signName) ? null : this.constractForm.signName,//签署人名称
        signMobile:  this.judge(this.constractForm.signMobilephone) ? null : this.constractForm.signMobilephone,//签署人手机号
        signTime:  this.judge(this.constractForm.signTime) ? null : this.$moment(this.constractForm.signTime).format("YYYY-MM-DD HH:mm:ss"),//签约时间
        contractName: this.contractName,//上传合同名称
        remark: this.judge(this.constractForm.contractNote) ? null : this.constractForm.contractNote,//合同备注
        legalName: this.judge(this.userForm.ceoName) ? null : this.userForm.ceoName,//法人姓名
        legalMobile: this.judge(this.userForm.ceoPhone) ? null : this.userForm.ceoPhone,//法人手机号
        treasurerName: this.judge(this.userForm.cfoName) ? null : this.userForm.cfoName,//财务主管名称
        treasurerMobile: this.judge(this.userForm.cfoPhone) ? null : this.userForm.cfoPhone,//财务主管手机号
        contractNoHead : this.draftStatus == 0 ? this.draftContractNo : this.constractForm.contactNoTitle == 0 ? this.constractForm.contactTitle: this.contactTitle,
      } 
      if(this.whichselet == 0){
        param.handlerId = this.judge(this.constractForm.auditor) ? null : this.constractForm.auditor
        param.handlerName = this.judge(this.$refs.myselect.selected.label) ? null : this.$refs.myselect.selected.label
      }else if(this.whichselet == 1){
        param.handlerId = this.judge(this.constractForm.auditor) ? null : this.constractForm.auditor[1]
        param.handlerName = this.judge(this.constractForm.auditor) ? null : this.constractForm.auName
      }else{
        param.handlerId = this.constractForm.userId
        param.handlerName = this.constractForm.name
      }
      if( this.$route.query.id != ""|| this.$route.query.id != null|| this.$route.query.clueId != null){
        param.id = Number(this.$route.query.id)
      }
      ...
      if(isDraft){
        ...
        if(this.judge(param.id)||this.judge(this.orderParam.clueId)){
          this.$message({
            type: 'warning',
            message: '请选择线索',
            offset: 100,
        ...
        saveContactDraft(param).then(res=>{
          ...
          if(this.$route.query.source == 'list'){
            clearDetail("/contracts/contracts-manage");
            this.$router.push("/contracts/contracts-manage")
          }else if(this.$route.query.source == 'personal'){
            clearDetail("/personal/my-contracts");
            this.$router.push("/personal/my-contracts")
          }
        })
    ...
    orderInfoList(val) {
      if(this.form.orderList.length>0){
        this.form.orderList.push(...Object.values(val));
      }else{
      ...
      // 数组去重
      this.orderIdList =  this.orderIdList.length>0 && [...new Set(this.orderIdList)]
    },
    // 添加订单
    addProduct() {
      if(Object.keys(this.clueInfo).length == 0){
