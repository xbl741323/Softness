...
<template>
  <div class="quick-tool">
    ...
    <strange-visit ref="visit"></strange-visit>
  </div>
</template>
...
import  strangeVisit from "@/components/staff/strange-visit/index";
...
export default { 
  ...
  methods: {
    ...
    viewMore(){
      ...
      this.viewNum = this.viewMoreStatus ? 10 : 55;
      this.menuList = filterPer(JSON.parse(JSON.stringify(homeData.menuList))).slice(0, this.viewNum);
    },
    routeLink(path){
    ...
    showStrangeVisit(){
      if(this.$refs.visit){
        this.$refs.visit.visitDialog = true
      }
    },
    stateBool(){
    ...
    toReceiveList() {
      this.permissions.bt_receive_amount_flag ? this.$router.push("/receivable/receivable-management") : this.$notAuthorized()
