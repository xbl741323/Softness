      ...
      <el-form :inline="true" :model="filters" ref="filters" @submit.native.prevent>
        ...
        <div v-if="senior">
          <el-form-item
            ...
            prop="policyType"
          >
            <el-select v-model="arr[index]" @change="addPolicyParam" value-key="id" :multiple='item.choice == 1?true:false'>
              <el-option
        ...
        <avue-crud
          ...
          <template slot="showStatus" slot-scope="scope">
            ...
            >
              <el-option :label="$t('status.show')" :value="1"></el-option>
              <el-option :label="$t('status.noShow')" :value="2"></el-option>
              <el-option :label="$t('status.toped')" disabled :value="0"></el-option>
            </el-select>
          </template>
          <!-- 级别 -->
          ...
          <template slot="showStatusEnum" slot-scope="scope">
            <span v-if="scope.row.isDraft == 0">{{ scope.row.showStatusEnum }}</span>
            <span v-if="scope.row.isDraft == 1">{{ scope.row.isDraft|drafFilter }}</span>
          </template>
...
import {
  ...
  batchDelete,
  listChangeStatus
} from "@/api/personal/policy";
import { getParamList } from "@/api/policy/param";
import * as productData from '@/views/project/project-data';
import { getNewArea } from "@/api/policy/list";
import { tableOption } from "@/const/crud/personal/policy";
...
export default {
  ...
  computed: {
    ...mapState({
      policySelection: (state) => state.category.policySelection,
  ...
  created(){
    if(sessionStorage.getItem(`${this.$route.path}+policy`)){
      let keepParams  = JSON.parse(sessionStorage.getItem(`${this.$route.path}+policy`))
  ...
  methods: {
    ...
    deleteBatch() {
        this.$confirm(this.$t('message.choosed')+`${this.delList.length}`+this.$t('message.essayConfirmDelete'), {
        confirmButtonText: this.$t("button.confirm"),
        cancelButtonText: this.$t("button.cancel"),
        cancelButtonClass: "btn-custom-cancel",
        type: "warning"
    ...
    getList() {
      ...
      let keepParams = {
        ...
        senior:this.senior,
        princeRadio:this.princeRadio,
        princeChild:this.princeChild,
        princeCopyChild:this.princeCopyChild,
        arr:this.arr,
      ...
      sessionStorage.setItem(`${this.$route.path}+policy`,JSON.stringify(keepParams))
      // 需要缓存的参数
      let keepParams2 = {
        detail:{path:"",query:{}}
      }
      ...
      getMyPolicyList(params).then(res => {
        ...
        let otherOptions = [
          ...
          {label: '汇编类型', prop:'assemblyType',slot:true,overHidden: true},
          ]
    ...
    getDropdown() {
      this.belongData = this.policySelection;
    ...
    handleDelete(row, index) {
      this.$confirm(
        this.$t('message.deleteConfirm') + row.number +  this.$t('message.isEssay'),
