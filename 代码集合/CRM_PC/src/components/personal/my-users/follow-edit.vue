...
<script>
import {getFollowup,comFollowup,upLoadFollowFile} from '@/api/personal/myusers'
import {getEditList} from '@/api/admin/user'
  ...
  props:{
   ...
   methods:{
    ...
    setInfo(){
      ...
      let params = {
        files: this.fileList,
        nextFollowTime: this.$moment(this.addForm.followNext).format("YYYY-MM-DD HH:mm"),
    ...
    setValue(){
        this.addForm.followType = this.userInfo.followTypeId;
        this.addForm.followContent = this.userInfo.followContent;
        this.addForm.followNext = this.userInfo.nextFollowTime;
        this.userInfo.files.forEach(item=>{
          this.fileList.push({
            url:  item.url,
            name: item.name,
