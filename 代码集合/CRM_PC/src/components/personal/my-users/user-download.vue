...
<!-- 排查完成 -->
  <div class="order-records">
    ...
    <avue-crud
      ...
      <!-- 端口 -->
      <template slot="downloadClient"  slot-scope="scope">
        <span>{{scope.row.downloadClient | clientFilter}}</span>
      </template>
    </avue-crud>
  </div>
</template>
...
<script>
import { getDownloadList  } from "@/api/admin/user";
import { getEmpInfo } from "@/api/project/wtClass";
import { tableOption } from "@/const/crud/user/user-download";
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      let params = {
        ...
        fileName: this.filters.keyWords,
