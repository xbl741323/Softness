<template>
  <div class="app-container calendar-list-container full-page" v-loading.fullscreen.lock="fullscreenLoading">
      ...
      <div class="btn-contain" v-if="deleted==0 && isEdit == 1">
  ...
  methods: {
    ...
    deleteFileTemp(row){
      this.$confirm('确认删除'+row.name+'吗？','删除',{
        ...
        this.filesTempListLength = this.filesTempList.length;// 删除后的文件数量
      })
    },
    deleteFileId(row){
      this.$confirm('确认删除'+row.name+'吗？','删除',{
    ...
    getDetailData() {
      getDetail(this.number).then(res => {
          ...
          this.selectProList = [];
          res.data.data.prods.forEach(item => {
            this.selectProList.push({
              id: item.spuId,
              spuTitle: item.spuTitle,
              policyId: item.policyId,
              policySpuId: item.policySpuId,
            })
          })
