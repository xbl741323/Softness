<template>
  <div class="banner">
    ...
    <el-table border :data="bannerList" ref="imgList" highlight-current-row style="width:100%">
      ...
      <el-table-column :label="$t('label.urlTxt')" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('label.opration')" align="center">
        <template slot-scope="scope">
          <!-- +编辑操作的dialog -->
          <el-dialog
...
import {
  ...
  getBannerList,
  moveBanner
} from "@/api/js-wotao-b/banner";
import upImg from "./upImg";
import * as CodeMsg from "@/api/common/CodeChange";
export default {
  props: ["type", "deviceType","substationId"],
  components: {
    upImg
  },
  data() {
  ...
  methods: {
    ...
    getList() {  
      getBannerList({type: this.type, substationId: this.substationId}).then(res => {
       if(res.data.code == CodeMsg.CODE_1000){
      ...
      this.$refs["bannerInfo"].validate(val => {
        if (val) {
          if (this.file !== "") {
            ...
            this.param.append("substationId", this.substationId);
    ...
    editBanner(row) {
      this.$refs["bannerInfo"].validate(val => {
        if (val) {
          this.param = new FormData();
          this.param.append("number", this.recordNumber);
          this.param.append("type", this.type);
