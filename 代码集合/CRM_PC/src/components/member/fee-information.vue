<template>
  <div>
    ...
    <el-table ref="multipleTable" :data="orderList" border show-summary :summary-method="getTotal">
      ...
      <el-table-column label="支付状态" align="center">
        <template slot-scope="scope">
            <div v-show="scope.row.payStatus==2" class="pay-column-sty">
              <span>支付成功</span>
              <span class="pay-record-sty" @click="showPayRecord()">付款记录</span>
            </div>
            <div v-show="scope.row.payStatus==0||scope.row.payStatus==1"><span>待付款</span></div>
            <div v-show="scope.row.payStatus==3"><span>支付失败</span></div>
            <div v-show="scope.row.payStatus==4"><span>取消失败</span></div>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
     ...
     :before-close="handleClose">
      <div class="pay-content">
        <span>支付方式：{{orderInfo.payType | payType}}</span>
        <span>支付时间：{{orderInfo.payTime}}</span>
        <span v-if="orderInfo.payType == 0">支付宝流水号： {{orderInfo.tradeNo}}</span>
        <span v-if="orderInfo.payType == 2">微信流水号： {{orderInfo.tradeNo}}</span>
        <span v-if="orderInfo.payType == 1" class="active-transfer" @click="showTransferImg()">查看转账凭证</span>
      </div>
     </el-dialog>
