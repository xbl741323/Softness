...
(function () {
    ...
    var countWords = function (node, schema,editor) {
      // var text = getText(node, schema).join('\n');
      // return getWords$1(text, identity).length+"/"+editor.getParam('ax_wordlimit_num', false );  //2020.11.19注释，防止编辑器不统计符号数字等
      ...
      // if( strLen(text) > ax_wordlimit_num){ //2020.11.19 获取编辑器统计的字符数量 
          ax_wordlimit_callback(editor, text, ax_wordlimit_num);
      // }else{
        // ax_wordlimit_callback(editor, text, ax_wordlimit_num);
      // }


      return strLen(text)+"/"+editor.getParam('ax_wordlimit_num', false );
    };
    var countCharacters = function (node, schema,editor) {
      var text = getText(node, schema).join('');
      return strLen(text)+"/"+editor.getParam('ax_wordlimit_num', false );
    };
    ...
    var createBodyCounter = function (editor, count) {
      return function () {
        return count(editor.getBody(), editor.schema,editor);
      };
    };
    var createSelectionCounter = function (editor, count) {
      return function () {
        return count(editor.selection.getRng().cloneContents(), editor.schema,editor);
      };
    };
    ...
    var register = function (editor, api) {
      editor.ui.registry.addButton('wordcountdisplay', {
        tooltip: 'Word count',
      ...
      editor.ui.registry.addMenuItem('wordcountdisplay', {
        text: 'Word count',
        icon: 'character-count',
        onAction: function () {
          return open(editor, api);
    ...
    function Plugin (delay) {
      ...
      global.add('wordcountdisplay', function (editor) {
        var api = get(editor);
        register(editor, api);
