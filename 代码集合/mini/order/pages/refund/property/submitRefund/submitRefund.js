...
Page({
  ...
  submitRefund() {
    ...
    if (serviceList[0].checked) {
      refundMessages.push({
        payType: refundInfo.payType,
        policyFeeType: 3, // 3-服务费
        refundAmount: serviceList[0].value
      })
    }
    if (refundInfo.isHaveGover == true && serviceList.length > 1 && serviceList[1].checked) {
      refundMessages.push({
        payType: refundInfo.payType,
        policyFeeType: 3, // 2-官费-原先传2现在固定传3
  ...
  getRefundInfo(options) {
    ...
    if (refundInfo.isHaveGover) {
      ...
      serviceList[0].value = (Number(refundInfo.nowFee) * refundInfo.productCount)
      serviceList[0].maxValue = (Number(refundInfo.nowFee) * refundInfo.productCount)
      serviceList[0].checked = refundInfo.isWaitSign == true ? true : false
      serviceList[1].value = (Number(refundInfo.governmentFee) * refundInfo.productCount)
      serviceList[1].maxValue = Number(refundInfo.governmentFee * refundInfo.productCount)
  ...
  fixMaxValue() {
    ...
    if (refundInfo.isHaveGover) {
      // 有官费
      serviceList = serviceList.slice(0, 2)
      serviceList[0].value = (Number(refundInfo.nowFee) * refundCount)
      serviceList[0].maxValue = (Number(refundInfo.nowFee) * refundCount)
      serviceList[1].value = Number(refundInfo.governmentFee * refundCount)
      serviceList[1].maxValue = Number(refundInfo.governmentFee * refundCount)
    } else {
      // 没有官费
      serviceList = serviceList.slice(0, 1)
      serviceList[0].value = (Number(refundInfo.nowFee) * refundCount)
