...
Page({
  data: {
    ...
    /** 是否已跳转认证小程序 */
    goFaceDone: false,
    redirectUrl: app.globalData.approveUrl,    
  },
  ...
  onShow: function () {
    ...
    if (options.scene === 1038 && options.referrerInfo.extraData && options.referrerInfo.extraData.faceResult) {
      ...
      if (previous.reloadPage && typeof previous.reloadPage === 'function') {
        console.log(miniProgramCallBackUrl,'miniProgramCallBackUrl--middle');
        let cuts = miniProgramCallBackUrl.indexOf('.com');
        let backUrl = this.data.redirectUrl + miniProgramCallBackUrl.slice(cuts + 4)
        previous.reloadPage(backUrl)        
        // previous.reloadPage(miniProgramCallBackUrl.replace("https://realnameverify.fadada.com",this.data.redirectUrl))        
        // beforePage.getInfo();
        wx.navigateBack({
          delta: 1,
        })
