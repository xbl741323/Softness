        ...
        <div class="cotent">
            ...
            <template v-if="showResult==true">
                <div class="contentresult">
                    ...
                    <p class="content-tip">你可以随时进入“<span @click="jump('/member/order')">我的订单</span> ”查看订单详情</p>
                </div>
            </template>
        </div>
    </div>
...
import { getWeiXinResult } from '~/api/order'
import { submitPay } from '~/api/member'
export default {
  head() {
    return {
      title: '',
  ...
  methods:{
    ...
    getOzhifuStatus(){
      ...
      getWeiXinResult({id: this.id}).then(res=>{
        if(res.data.code==0){
