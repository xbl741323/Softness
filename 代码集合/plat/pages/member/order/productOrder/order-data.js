const statusList=[
    ...
    {value:1, label:'待签约',orderStatus: 1},
    {value:0, label:'待付款',orderStatus: 0},
    {value:2, label:'服务中',orderStatus: 3},
    {value:99, label:'退款/售后',orderStatus: 99},
]
...
export function orderStatus(val){
    ...
    // return val == 0 ? '服务中' : val == 1 ? '待分配' : val == 2 ? '服务中' : val == 3 ? '冻结中' : val ==4 ? '已完成' : val == 5 ? '交易关闭' : val == 6 ? '待付款' : val == 7 ? '已取消' : val ==8 ? '已删除' : val==9? '待签约':''
}
