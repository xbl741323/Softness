      ...
      <div class="unit_form">
        <el-form :model="unitForm" ref="unitForm" :rules="unitRules" label-width="150px" size="medium">
          ...
          <el-form-item label="单位名称" prop="orgName">
            <el-input placeholder="请输入单位名称" maxlength="50" v-model.trim="unitForm.orgName" />
          </el-form-item>
          <el-form-item label="统一社会信用代码" prop="creditCode">
            <el-input placeholder="请输入真实统一社会信用代码" maxlength="50" :disabled="disabled" v-model.trim="unitForm.creditCode" />
          </el-form-item>
...
import { againUpdateCompanyAuth, getAuthStatus, updateCompanyAuth } from "@/api/auth";
    ...
    var codeValidator = (rule, value, callback) => {
      if (value && Number(value) !== 0) {
        ...
        if (pattern.test(value)) {
          ...
          callback(new Error("请输入真实统一社会信用代码或检查是否存在空格"));
        }
      } else {
        callback(new Error("请输入真实统一社会信用代码"));
      }
    };
      ...
      this.$refs.unitForm.validate((valid) => {
        if (valid) {
          let param = {
            ...
            verifiedWay: this.unitForm.authWay, // 授权方式 1：对公打款；2:纸质材料认证 3：法人身份（授权）认证
          ...
          if(this.status == '2' && this.isAgain == false) {
            ...
            againUpdateCompanyAuth(param).then(res => {
              if(res.data.code != CodeMsg.CODE_0){
                if(res.data.code == 10002){
                  localStorage.removeItem('userInfo');
                  location.href = "/login";
