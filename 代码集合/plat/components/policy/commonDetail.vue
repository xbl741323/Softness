  ...
  <client-only>
    <div class="frame">
      ...
      <div class="details-content">
        <div v-if="collectStatus" :class="[proList&&proList.length>0?'left-details':'left-detaill']">
          ...
          <div class="mar">
            ...
            <div v-if="this.fileLength > 0" class="attachment">
              ...
              <div>
                 <div class="document_list" v-if="dataList.filesList&&dataList.filesList.length>0">
                  <div class="item" v-for="(citem, cindex) in dataList.filesList" :key="cindex" @click="goDocDetail(citem)">
                    <div class="img_box">
                      <img :src="cdnUrl +citem.fileImageUrl" alt class="img_sty">
                      <img v-if="fileTypeArr.indexOf(citem.fileType)>-1" :src="iconUrl[citem.fileType]" alt class="icon_type">
                      <span class="icon_buy" v-if="citem.isPay==1">付费</span>
                    </div>
                    <el-tooltip
                      v-if="citem.fileName&&citem.fileName.length>15"
                      content="Top"
                      placement="top"
                      effect="light"
                      >
                      <span slot="content">{{citem.fileName}}</span>
                      <span class="tit">{{citem.fileName}}</span>
                    </el-tooltip>
                    <span class="tit" v-else>{{citem.fileName}}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="right-details" v-if="proList.length==0">
          <div class="right_content">
            <div>
              <right-side :typeId="this.policyType"></right-side>
            </div>
          </div>
      </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    toLast(number,id) {
      // 上下篇切换
      if(number.indexOf('TZ')>-1){
        window.open(number + '01?id='+id, '_self')
      }else{
        window.open(number + '101?id='+id, '_self')
      }
    },
    // 跳转到对应列表
    toList(data) {
