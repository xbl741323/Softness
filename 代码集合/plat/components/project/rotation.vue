...
<script>
import { views } from '@/api/announcement'
export default {
  props: ["coverUrls"],
  data() {
    return {
  ...
  created(){
    views(this.$route.params.projectDetail.substr(0,this.$route.params.projectDetail.length-1)).then(res=>{
  ...
  methods: {
    ...
    qqShare() {
     const urlData =
        ...
        '&sharesource=qzone&title=项目申报&pics=你的分享图片&summary=你的分享描述信息'
      window.open(urlData, '_blank')
    },
    //微博分享
    weiboShare() {
     const urlData =
        'http://service.weibo.com/share/share.php?&sharesource=weibo&title=项目申报&url=' +
        window.location.href +
