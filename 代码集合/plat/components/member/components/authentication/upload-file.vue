<template>
    ...
    <div v-if="imageUrl.length > 0">
      <div :style="imgStyle" v-if="type == 'application/pdf'" >
        <img style="width:80px;height:80px;margin-top:15%" src="~assets/images/member/pdf.png" />
  ...
  export default {
    ...
    data() {
      return {
        ...
        type: '',
        url: '',
        acceptFileType: "image/png, image/jpeg, image/png, application/pdf"
      };
    },
    computed:{
      imgStyle(){
      ...
      iconStyle(){
        ...
        iconStyle.marginTop = this.height/3+ 'px';
        return iconStyle;
      },
    },
    methods: {
      onChange(file){
        ...
        const isAcceptFile = this.acceptFileType.match(reg);
