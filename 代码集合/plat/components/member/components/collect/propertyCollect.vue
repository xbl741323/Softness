...
<script>
import { getCollectList, batchDelete,getResultCollectList,getCountResultCollectList,cancelResult } from "@/api/member";
import { getProCollectList,cancelMultiPropertyCollect,getCollectionStatistic} from "@/api/property";
import { getUserInfo } from "@/api/login";
import resultItem from "./propertyItem";
export default {
  components: {
    resultItem,
  },
  ...
  methods: {
      ...
      if (this.soloCheckList.length > 0) {
        this.$confirm("确定删除所选的收藏吗？", "提示", {
          ...
          let params = {
            ...
            cancelMultiPropertyCollect(params).then(res=>{
                if(res.data.code == 0){
                 this.$message({
                type: "success",
                message: "删除成功！",
    ...
    getList() {
      ...
      getCollectionStatistic(param).then((res) => {
