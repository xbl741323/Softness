// personal/components/my-clue/clue-info/clue-info.js
const app = getApp();
import {
  getFinanceInfo
} from "../../../../utils/api/clue"
...
Component({
  ...
  methods: {
    bindYearChange(e){
      ...
      this.triggerEvent('getCurrentYear', this.data.currentYear);
      this.getFinance();
    },
    getFinance(){
      this.setData({
    ...
    */
    ...
    sendAuthorization(e) {
      ...
      if (item.legalMobileFlag) {
        // 校验通过-跳转到发起授权页面
        wx.navigateTo({
          url: `/personal/pages/my-clue/launch-accredit/launch-accredit?id=${data.clueCompanyId}&&chooseId=${item.id}&&clueType=1&&organization=${data.organization}&&certifiedLabel=${this.data.certifiedLabel}`,
        })
      } else {
        wx.showToast({
          icon: 'none',
          title: '手机号码不合规，请修改后再试！',
        })
      }
    },
    // 打电话
