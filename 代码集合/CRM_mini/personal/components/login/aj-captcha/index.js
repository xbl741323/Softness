...
Component({
  ...
  methods: {
    ...
    _blockPuzzleCheck(obj) {
      ...
      let url = _self.data.opt.baseUrl + `/code/check?captchaType=${data.captchaType}&pointJson=${encodeURIComponent(data.pointJson)}&token=${data.token}`;
      _self._postData(url, function (res) {
        ...
        if (res.data.repCode == "0000") {
          ...
          setTimeout(function () {
            ...
            if (typeof _self.data.opt.success == "function") {
              _self.data.opt.success({
                'captchaVerification': captchaVerification
              });
            }
            _self.triggerEvent('verifySuccess', {
              captchaVerification
            })
          }, 700);
        } else {
    ...
    _setPoint(detail_x, detail_y) {
      ...
      pointList[_self.data.clickWordTapNum] = {
        left: (offsetX - 10) + "px",
        top: (offsetY - 10) + "px",
        display: "block"
      };
    ...
    _refresh() {
      let _self = this;
      let url = _self.data.opt.baseUrl + "/code";
      let clientUid = "";
      ...
      _self._postData(url, function (res) {
        console.log(res, "refreshrefreshrefresh")
        if (res.data.repCode == "0000") {
