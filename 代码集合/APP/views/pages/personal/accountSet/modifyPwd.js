...
export default function ModifyPwd(props) {
  ...
  const [newPwdBlur, setNewPwdBlur] = useState(false) // 密码是否通过校验 
  const [checkNewPwdBlur, setCheckNewPwdBlur] = useState(false) // 新密码是否通过校验 
  ...
  const checkNewPwd = () => {
    ...
    if (reg.test(formInfo.newPwd)) {
      formInfo.newPwdError = false
      setFormInfo({ ...formInfo })
    } else {
      formInfo.newPwdError = true
      setFormInfo({ ...formInfo })
    }
  }
  ...
  // 确认新密码必填项校验
  const checkConfirmNewPwd = () => {
    setCheckNewPwdBlur(true)
    if (Number(formInfo.checkNewPwd) != 0) {
      formInfo.checkNewPwdError = false
      setFormInfo({ ...formInfo })
    } else {
      formInfo.checkNewPwdError = true
      setFormInfo({ ...formInfo })
    }
  }
  ...
  // 修改新密码值
  const changeNewPwdInput = (val) => {
    formInfo.newPwd = val.trim()
    setFormInfo({ ...formInfo })
  }

  // 修改确认新密码值
  const changecheckNewPwdInput = (val) => {
    formInfo.checkNewPwd = val.trim()
    setFormInfo({ ...formInfo })
  }
  ...
  // 调用接口判断验证码是否正确
  const passCodeValid = () => {
    smsCheckForManager(formInfo.code).then(res => {
      if (res.code == CodeMsg.CODE_0) {
        setPassValid(true)
      } else {
        Toast.show(res.msg, {
  ...
  const finalValid = throttle(() => {
    if (formInfo.newPwdError) {
      checkNewPwd()
    }
    if (formInfo.checkNewPwdError) {
      checkConfirmNewPwd()
    }
    if (!formInfo.newPwdError && !formInfo.checkNewPwdError) {
      console.log('校验通过')
      passFinalConfirm()
    }
  }, 1000)
  ...
  const passFinalConfirm = () => {
    if (formInfo.newPwd == formInfo.checkNewPwd) {
      let params = {
        passWord: MD5.hex_md5(formInfo.newPwd)
      };
        ...
        {passValid ?
          (<View style={styles.modifyForm}>
            <View style={styles.modifyFormItem}>
              <Text style={styles.modifyLabel}>新密码：</Text>
              <View style={[styles.codeContain, styles.finalCodeText]}>
                <TextInput keyboardType="default" autoCapitalize={'none'} secureTextEntry={true} value={formInfo.newPwd} onBlur={(e) => checkNewPwd(e)} onChangeText={(e) => changeNewPwdInput(e)} style={[styles.modifyInput, formInfo.newPwdError && newPwdBlur ? styles.formErrorSty : '', styles.finalText]} placeholder='设置密码（请使用两种或两种以上字符组合'></TextInput>
                {formInfo.newPwdError && newPwdBlur ? (<Text style={styles.errorText}>请输入6~16位字母、数字和符号两种以上组合</Text>) : ''}
              </View>
            </View>
            <View style={styles.modifyFormItem}>
              <Text style={styles.modifyLabel}>确认新密码：</Text>
              <View style={[styles.codeContain, styles.finalCodeText]}>
                <TextInput keyboardType="default" autoCapitalize={'none'} secureTextEntry={true} value={formInfo.checkNewPwd} onBlur={(e) => checkConfirmNewPwd(e)} onChangeText={(e) => changecheckNewPwdInput(e)} style={[styles.modifyInput, formInfo.checkNewPwdError && checkNewPwdBlur ? styles.formErrorSty : '', styles.finalText]} placeholder='请再次确认密码'></TextInput>
                {formInfo.checkNewPwdError && checkNewPwdBlur ? (<Text style={styles.errorText}>请再次输入新密码</Text>) : ''}
              </View>
            </View>
          </View>
          ) : (<View>
